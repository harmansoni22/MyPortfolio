...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getElementCenter(element) {
      ...
      const centerX = scaleByPixelRatio(rect.left + rect.width / 2);
      const centerY = scaleByPixelRatio(rect.top + rect.height / 2);
      if (canvas.width === 0 || canvas.height === 0) return null;

      return {
        x: clamp(centerX / canvas.width, 0.001, 0.999),
        y: clamp(1.0 - centerY / canvas.height, 0.001, 0.999),
        width: rect.width,
        height: rect.height,
      };
    }

    function emitBurstFromElement(
      element,
      forceMultiplier,
      burstCount,
      burstSpread,
      ...
      const baseForce = config.SPLAT_FORCE * forceMultiplier;
      const color = generateColor();
      color.r *= 10 * forceMultiplier;
      color.g *= 10 * forceMultiplier;
      color.b *= 10 * forceMultiplier;

      const spreadX =
        scaleByPixelRatio(Math.max(10, center.width * burstSpread)) /
        canvas.width;
      const spreadY =
        scaleByPixelRatio(Math.max(10, center.height * burstSpread)) /
        canvas.height;
      const ringForce = baseForce * 0.14;
      const centerForce = baseForce * 0.22;
      const burstPoints = [
        {
          x: 0,
          y: 0,
          dx: (Math.random() - 0.5) * centerForce,
          dy: (Math.random() - 0.5) * centerForce,
          tint: 1,
        },
        {
          x: -spreadX,
          y: 0,
          dx: -ringForce,
          dy: (Math.random() - 0.5) * ringForce * 0.35,
          tint: 0.75,
        },
        {
          x: spreadX,
        ...
        },
        {
          x: 0,
          y: -spreadY,
          dx: (Math.random() - 0.5) * ringForce * 0.35,
          dy: -ringForce,
          tint: 0.75,
        },
        {
      ...
      burstPoints.slice(0, burstCount).forEach((point) => {
        const tintedColor = {
          r: color.r * point.tint,
          g: color.g * point.tint,
          b: color.b * point.tint,
        };
        splat(
          clamp(center.x + point.x, 0.001, 0.999),
          clamp(center.y + point.y, 0.001, 0.999),
          point.dx,
          point.dy,
          tintedColor,
        );
    ...
    //   if (
    //     relatedTarget &&
    //     typeof interactiveElement.contains === "function" &&
    //     interactiveElement.contains(relatedTarget)
    //   ) {
    //     return;
    //   }
    ...
    //   const now = Date.now();
    //   if (
    //     interactiveThrottleMs > 0 &&
    //     now - lastInteractiveHoverTime < interactiveThrottleMs
    //   )
    //     return;

    //   lastInteractiveHoverTime = now;
    //   emitBurstFromElement(interactiveElement, interactiveHoverForce, 3, 0.08);
    // }

    function handleInteractiveClick(event) {
      const interactiveElement = getInteractiveElement(event.target);
      if (!interactiveElement) return;
      emitBurstFromElement(interactiveElement, interactiveClickForce, 5, 0.1);
    }

    function handleInteractiveFocus(event) {
      const interactiveElement = getInteractiveElement(event.target);
      if (!interactiveElement) return;
      emitBurstFromElement(interactiveElement, interactiveFocusForce, 3, 0.06);
    }
    ...
    const hoverEventName = window.PointerEvent ? "pointerover" : "mouseover";
    ...
    // document.addEventListener(hoverEventName, handleInteractiveHover, true);
    ...
    return () => {
      ...
      document.removeEventListener("focusin", handleInteractiveFocus, true);
    };
  }, [
    SIM_RESOLUTION,
    DYE_RESOLUTION,
